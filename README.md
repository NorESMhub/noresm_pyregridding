...
Then run 
```
python regrid_all_files_in_folder.py --inputdir raw_data_folder_path --outputdir path_to_dump_output --realm {atm, lnd} --inputres {ne16, ne30} --workers number_of_workers (default is 1)
```
where `raw_data_folder_path` is the path to the raw output you want to regrid (typically lnd/hist or atm/hist folders), `path_to_dump_output` is the path to dump output, the regridding can be run for either the `atm` (cam) or the `lnd` (ctsm) realm (component) and for either the `ne16` or the `ne30` resolution.
